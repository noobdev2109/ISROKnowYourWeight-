...
function calculateWeight(weight, gravity) {
    const weightInNewtons = weight * gravity ;
    return weightInNewtons.toFixed(2);
}

function displayWeight() {
    const planetSelect = document.getElementById("planetSelect");
    const selectedPlanet = planetSelect.value;
    const gravity = parseFloat(planetSelect.options[planetSelect.selectedIndex].getAttribute("data-gravity"));

    const weight = parseFloat(document.getElementById("weightInput").value);
    const planetWeight = calculateWeight(weight, gravity);

    document.getElementById("result").innerHTML = `Weight on ${selectedPlanet}: ${planetWeight} N`;
}
